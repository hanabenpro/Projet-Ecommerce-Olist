...
INSERT INTO dbo.fact_orders (
    ...
    shipping_limit_date, price, freight_value, payment_value, payment_type,
    review_score, delivery_delay_days, delivery_time_days, is_late_delivery, total_revenue
)
SELECT
    o.order_id,
    oi.order_item_id,
    ...
    DATEDIFF(day, oi.shipping_limit_date, o.order_delivered_customer_date) AS delivery_time_days,
...
FROM
    dbo.stg_orders_clean o
JOIN
    dbo.stg_order_items_clean oi ON o.order_id = oi.order_id
LEFT JOIN
   dbo.stg_reviews_clean r ON o.order_id = r.order_id
JOIN 
    dbo.stg_payments_clean p ON o.order_id = p.order_id;
